...
def main(cfg, gpu, save_dir):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        pbar = tqdm(enumerate(trainloader), total=iters_per_epoch,
                    desc=f"Epoch: [{epoch + 1}/{epochs}] Iter: [{0}/{iters_per_epoch}] LR: {lr:.8f} All Loss: {all_loss:.8f} Our Loss: {all_loss_pcumd:.8f}.")
        ...
        for iter, (sample, lbl) in pbar:
            ...
            with autocast(enabled=train_cfg['AMP']):
                ...
                loss_all = loss_s + loss + loss_pcumd
            ...
            pbar.set_description(
                f"Epoch: [{epoch + 1}/{epochs}] Iter: [{iter + 1}/{iters_per_epoch}] LR: {lr:.8f} All Loss: {all_loss / (iter + 1):.8f} Our Loss:{all_loss_pcumd / (iter + 1): .8f}")
        ...
        if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
            ...
            writer.add_scalar('train/loss_pumd', all_loss_pcumd, epoch)
