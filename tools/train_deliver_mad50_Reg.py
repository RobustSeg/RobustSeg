...
def main(cfg, gpu, save_dir):
    ...
    model = Seg("mit_b0", num_classes=trainset.n_classes, pretrained=True, modals=dataset_cfg['MODALS'])

    model_s = Seg_s("mit_b0", num_classes=trainset.n_classes, pretrained=True, modals=dataset_cfg['MODALS'])
    # model_path = "/home/dell/PythonProjects/DELIVER/output/Deliver_teacher/DELIVER_mit_b0_idel/Segformer_mit_b0_DELIVER_epoch141_61.92.pth"
    ...
    # 加载checkpoint的逻辑 =================================================================
    resume_path = cfg['MODEL']['RESUME']  # 从配置文件中获取checkpoint路径
    start_epoch = 0
    best_mIoU = 0.0
    ...
    if resume_path and os.path.isfile(resume_path):
        ...
        if list(state_dict.keys())[0].startswith('module.'):
            if not train_cfg['DDP']:  # 如果当前不是DDP模式，需要去掉前缀
                state_dict = {k[7:]: v for k, v in state_dict.items()}
        else:
            if train_cfg['DDP']:  # 如果当前是DDP模式，需要添加前缀
    ...
    for epoch in range(start_epoch, epochs):
        ...
        pbar = tqdm(enumerate(trainloader), total=iters_per_epoch,
                    desc=f"Epoch: [{epoch + 1}/{epochs}] Iter: [{0}/{iters_per_epoch}] LR: {lr:.8f} Loss: {all_loss:.8f}")
        ...
        for iter, (sample, lbl) in pbar:
            ...
            with autocast(enabled=train_cfg['AMP']):
                ...
                with torch.no_grad():
                    logits_t, ms_feat_t = model_s(sample)
                ...
                loss_s = loss_kl(logits, logits_t).clamp(min=0) * 50

                loss_reg = RUMD(index, ms_feat, ms_feat_t, reg_params) * 0.003
                loss_all = loss_s + loss + loss_reg
            ...
            all_loss_reg += loss_reg.item()

            pbar.set_description(
                f"Epoch: [{epoch + 1}/{epochs}] Iter: [{iter + 1}/{iters_per_epoch}] LR: {lr:.8f} All Loss: {all_loss / (iter + 1):.8f} Our Loss:{all_loss_reg / (iter + 1): .8f}")
        ...
        if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
            ...
            writer.add_scalar('train/loss_reg', all_loss_reg, epoch)
        torch.cuda.empty_cache()
        ...
        if ((epoch + 1) % train_cfg['EVAL_INTERVAL'] == 0 and (epoch + 1) > train_cfg['EVAL_START']) or (
            ...
            if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
                ...
                mean_miou = (miou_1 + miou_2 + miou) / 3
                ...
                logger.info(f"Current epoch:{epoch} mIoU: {miou} mean mIoU: {best_mIoU}")
